...
static void heap_monitor_task(void *arg)
{
    ...
    for (;;) {
        ...
        ESP_LOGI("HEAP", "internal free: %u B (%u KB) | largest block: %u B | min-ever: %u B (%u KB)",
                 (unsigned)free_int, (unsigned)(free_int / 1024),
                 (unsigned)largest, (unsigned)min_ever, (unsigned)(min_ever / 1024));
...
void app_main(void)
{
    ...
    /* start the internal-RAM heap monitor (prints every 5 s) */
