...
static float lhdc_dec_gain_exponent(int gain_idx, int enc_frame_len, int bit_depth)
{
...
#endif
    /* GenerateGainTable computes `split` differently per bit depth (verified from
     * the encoder disassembly): 16-bit divides by 2.5 with consts -0.017144/26.18;
     * 24-bit divides by 4 with consts -0.01705/26.20 (48k/44.1k). slope and the
     * idx>503 tail are the same shape. */
    int split;
    if (bit_depth == 24) {
        split = (int)(((L - 50) * (-0.01705f) + 26.20f) * 0.25f + 0.5f);   /* /4 -> *0.25 (exact) */
    } else {
        /* NOTE: kept as /2.5f, NOT *0.4f: 0.4f is not exactly representable and
         * this feeds an integer `split` that must match the encoder bit-for-bit;
         * a 1-ULP shift could flip split. Runs once per channel (~0% CPU). */
        split = (int)(((L - 50) * (-0.017144f) + 26.18f) / 2.5f + 0.5f);  /* round */
    }
    if (split < 1) split = 1;
    if (split > 30) split = 30;
    /* 24-bit: cap split at 5. Our formula over-produces split for small frames
     * (low bitrate): 256k/400k frames compute split=6, but the encoder uses ~5,
     * so those frames decoded ~1.8x too LOUD (e = split + (gain-split)*slope, and
     * a wrong split shifts the whole dequant scale). That made the volume jump
     * with bitrate and pushed loud low-bitrate content toward the conceal
     * threshold. Validated via real-encoder round-trip: at 400k, split 6->5 moves
     * the level 519->267, matching 500k(split5)/900k(split4) at ~280. 500k/900k
     * are already <=5 so they are unchanged. */
    ...
    /* 24-bit: the sloped-region gain step is a near-CONSTANT ~0.0378/index, NOT
     * the (e_full-split)/(503-split) curve. The curve over-slopes as `split`
     * shrinks (high bitrate -> small split): at 96k 900k it gave slope 0.042 vs
     * the true ~0.038, so the per-frame dequant scale was wrong whenever the
     * encoder's global_gain swings frame-to-frame (e.g. a steady 50 Hz tone,
     * gain alternating 31<->44). That mis-scale breaks MDCT TDAC cancellation
     * and left a -36 dB amplitude-modulation comb at the 200 Hz frame rate
     * (odd-harmonic sidebands) -- the "dirty/muffle, only at 96k" the user heard.
     * 48k was clean only because its gains happened to be stable (slope unused).
     * Calibrated against the real liblhdcv5.so encoder via round-trip over 96k
     * 256/500/900k + 192k: 0.0378 nulls 500k/900k to -71/-61 dB (was -41/-36)
     * with no regression at 256k/48k. (tools/lhdc_roundtrip, LHDC_SLOPE/EFULL.) */
