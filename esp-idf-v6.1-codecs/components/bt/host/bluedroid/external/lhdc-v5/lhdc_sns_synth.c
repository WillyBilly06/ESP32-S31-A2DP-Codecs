...
static void sns_post_smooth(int32_t *sf, int n)
{
    ...
    /*
     * The encoder NEGATES the smoothed envelope, then clamps to [-2560, 1024]
     * (verified bit-exact against liblhdcv5.so sns_encode post-processing:
     * recon -> mean-removal -> 4-tap smoothing -> negate -> clamp = sf_out, the
     * exact array passed to sns_apply). The clamp MUST follow the negation.
     */
    for (int i = 0; i < n; i++) {
        int v = -out[i];
        if (v < -2560) v = -2560;
